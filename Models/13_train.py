...
def main():
    ...
    def criterion(pred, target):
        return F.binary_cross_entropy_with_logits(pred, target, pos_weight=torch.tensor([1.5]).to(pred.device))
    ...
    for epoch in range(100):
        ...
        if f1 > best_f1:
            best_f1 = f1
            torch.save(model.state_dict(), "(11)ResNet_Trans_AttPool_best.pth")
