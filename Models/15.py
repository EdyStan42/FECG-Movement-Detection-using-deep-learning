...
def extract_features(sig, qrs_indices):
    ...
    return np.stack([linear_env, hilbert_env, residual, local_rms, qrs_rate, curvature], axis=0)
...
class AttentionUNet(nn.Module):
    ...
    def forward(self, x):
        ...
        return self.final(d1)  # (batch, 4, seq_len)
...
def main():
    ...
    print(f"Attention U-Net | Multiclass (4) | Step target | LR: {LEARNING_RATE}")
    best_f1 = 0.0
    ...
    for epoch in range(100):
        ...
        if macro_f1 > best_f1:
            best_f1 = macro_f1
            torch.save(model.state_dict(), "15_AttUNet_multiclass_best.pth")
