...
# Model 19: model 18 (signal-level norm, 3 features, bottleneck self-attention)
# with true window-level random shuffling.
#
# Problem with model 18's SignalLevelLoader: shuffling signals, not windows.
# All windows from one 20-minute recording arrive in ~2400 consecutive batches,
# so gradient updates are highly correlated → jagged training curves.
#
# Naive fix (PrecomputedLoader that stores all windows): requires ~44 GB RAM
# for 400 signals — not feasible.
#
# This solution: cache only the feature arrays (~2.9 GB for 400 signals),
# build a flat index of (signal_idx, start_sample) pairs, shuffle the index
# each epoch. Each __getitem__ slices the already-cached features — fast
# without storing every window explicitly.
...
def extract_features_signal(sig, qrs_locs):
    ...
    return np.stack([residual, local_rms, qrs_rate], axis=0).astype(np.float32)
...
# --- 5. DATA LOADER ---
class IndexedWindowLoader:
    """
    Window-level random shuffling without storing all windows in RAM.

    Startup: load all signals and precompute their feature arrays (3, N) —
    for 400 signals of 20 min at 500 Hz this is ~2.9 GB, feasible.

    Then build a flat index of (signal_idx, window_start) pairs covering every
    valid window across all signals. Each epoch shuffles this index with
    np.random.permutation, so every batch draws windows randomly from across
    all recordings and all time positions.

    get_batches() slices from the already-cached feature and mask arrays —
    no disk I/O after startup, no 44 GB allocation.
    """
    def __init__(self, sig_paths, mask_paths, qrs_paths, window_size, stride):
        self.window_size = window_size
        self.feats_list  = []
        self.masks_list  = []
        self.index       = []   # list of (sig_idx, start)

        total_gb = 0.0
        for i, (sp, mp, qp) in enumerate(zip(sig_paths, mask_paths, qrs_paths)):
            sig      = np.load(sp).flatten().astype(np.float64)
            mask     = np.load(mp).flatten()
            qrs_locs = np.load(qp).flatten().astype(int)
            feats    = extract_features_signal(sig, qrs_locs)   # (3, N) float32
            self.feats_list.append(feats)
            self.masks_list.append((mask, qrs_locs))
            total_gb += feats.nbytes / 1e9

            n = len(sig)
            for start in range(0, n - window_size, stride):
                self.index.append((i, start))

        self.index = np.array(self.index, dtype=np.int64)   # (N_windows, 2)
        print(f"  {len(sig_paths)} signals  |  {len(self.index):,} windows  "
              f"|  feature cache: {total_gb:.2f} GB")

    def get_batches(self, batch_size):
        perm = np.random.permutation(len(self.index))
        x_batch, y_batch = [], []

        for k in perm:
            sig_idx, start = self.index[k]
            end   = start + self.window_size
            feats, (mask, qrs_locs) = self.feats_list[sig_idx], self.masks_list[sig_idx]
...
def main():
    ...
    print(f"Model 19 | 3 features | signal-level norm | self-attention | window-level shuffle")
    print(f"Device: {device}  |  Params: {n_params:,}  |  LR: {LEARNING_RATE}")
    print(f"\nBuilding training index...")
    train_loader = IndexedWindowLoader(sig_p[:split], mask_p[:split], qrs_p[:split],
                                       WINDOW_SIZE, STRIDE)
    print(f"Building validation index...")
    val_loader   = IndexedWindowLoader(sig_p[split:], mask_p[split:], qrs_p[split:],
                                       WINDOW_SIZE, STRIDE)
    print()
    ...
    for epoch in range(EPOCHS):
        model.train()
        t_loss, t_steps = 0, 0
        for x, y in train_loader.get_batches(BATCH_SIZE):
        ...
        TP = FP = FN = TN = 0
        with torch.no_grad():
            for x, y in val_loader.get_batches(BATCH_SIZE):
                x, y = x.to(device), y.to(device)
                pred   = model(x)
        ...
        if f1 > best_f1:
            best_f1 = f1
            torch.save(model.state_dict(), "19_AttUNet_3feat_selfattn_winshuf_best.pth")
            print(f"--> New best (F1={f1:.3f})  saved: 19_AttUNet_3feat_selfattn_winshuf_best.pth")
