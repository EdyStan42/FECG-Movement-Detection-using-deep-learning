...
# Model 22: model 21 + moving-average baseline residual.
# Same QRS-free setup as model 21, but replaces the first-difference
# feature with a "residual" feature: sig_norm minus a moving-average
# baseline (window ~ one RR interval). This approximates model 18's
# QRS-anchored residual (sig - linear envelope through QRS peaks)
# without needing any peak locations.
#
# Features:
#   sig_norm   — signal-level normalised raw waveform
#   local_rms  — rolling RMS energy (50-sample window)
#   residual   — sig_norm minus moving-average baseline (200-sample window)
#
...
def extract_features_signal(sig):
    ...
    baseline  = np.convolve(sig_norm, np.ones(BASELINE_WINDOW) / BASELINE_WINDOW, mode='same')
    residual  = sig_norm - baseline
    residual  = (residual - np.mean(residual)) / (np.std(residual) + 1e-8)
    return np.stack([sig_norm, local_rms, residual], axis=0).astype(np.float32)
...
def main():
    ...
    print(f"Model 22 | 3 features (no QRS, MA-baseline residual) | signal-level norm | self-attention")
    ...
    for epoch in range(EPOCHS):
        ...
        if f1 > best_f1:
            best_f1 = f1
            torch.save(model.state_dict(), "22_AttUNet_3feat_maresidual_best.pth")
            print(f"--> New best (F1={f1:.3f})  saved: 22_AttUNet_3feat_maresidual_best.pth")
