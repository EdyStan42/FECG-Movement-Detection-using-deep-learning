...
# Model 20: model 18 (signal-level norm, 3 features) with bottleneck self-attention
# replaced by a causal (forward-only) GRU.
#
# Self-attention sees every position simultaneously — no notion of past vs future.
# A causal GRU processes left→right only: each compressed timestep accumulates a
# hidden state from all previous positions. This respects temporal causality and
# is compatible with real-time deployment where future samples are unavailable.
#
# Training uses signal-level shuffling (same as model 18).
# Comparison target: model 18 (self-attention, signal shuffle)
...
class BottleneckGRU(nn.Module):
    """
    Causal (forward-only) GRU over the bottleneck sequence (240 compressed timesteps).
    Each position attends only to past context — consistent with real-time deployment
    where future samples are not available.
    hidden_size=256, unidirectional → output is 256 channels, same as input.
    Residual + LayerNorm keeps gradient flow clean.
    """
    def __init__(self, channels=256):
        super().__init__()
        self.gru  = nn.GRU(channels, channels, num_layers=1,
                           batch_first=True, bidirectional=False)
        self.norm = nn.LayerNorm(channels)

    def forward(self, x):              # (B, C, L)
        x_t = x.permute(0, 2, 1)      # (B, L, C)
        out, _ = self.gru(x_t)        # (B, L, C)  — 128 fwd + 128 bwd = 256
        return self.norm(x_t + out).permute(0, 2, 1)   # (B, C, L)
...
class AttentionUNet(nn.Module):
    def __init__(self, in_channels=NUM_FEATURES):
        ...
        self.bottle_gru = BottleneckGRU(channels=256)
...
# --- 5. DATA LOADER (signal-level shuffling, same as model 18) ---
class SignalLevelLoader:
    """
    Shuffles the list of signals each epoch, then yields windows from each
    signal in order. This keeps temporal continuity within a signal intact,
    which matters for a GRU: hidden states built up during one window are not
    reused across windows here (each window is independent), but the signal
    ordering still controls how correlated consecutive batches are.
    """
...
def main():
    ...
    print(f"Model 20 | 3 features | signal-level norm | bottleneck GRU (causal) | signal-level shuffle")
    ...
    for epoch in range(EPOCHS):
        ...
        if f1 > best_f1:
            best_f1 = f1
            torch.save(model.state_dict(), "20_AttUNet_3feat_bigru_best.pth")
            print(f"--> New best (F1={f1:.3f})  saved: 20_AttUNet_3feat_bigru_best.pth")
