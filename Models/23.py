...
# Model 23: simplest possible QRS-free input — all 6 electrode channels,
# each contributing 2 features (sig_norm, local_rms), stacked as 12 input
# channels. One sample per recording (122 total), single shared mask as
# target.
#
# Rationale: feature-engineering attempts (channel diffs, RMS products,
# cross-channel spread, bandpass filtering, envelope smoothing) all showed
# weak/inconsistent correlation with the movement label across the full
# dataset. Let the network learn directly from all 6 channels at once.
# `residual` (sig_norm minus a 200-sample moving-average baseline) was
# dropped: measured correlation with sig_norm was 0.996 across the
# dataset, i.e. it's redundant given the baseline is tiny relative to
# the signal's variance for these noisy recordings.

NUM_FEATURES = 6 * 2


# --- 1. FEATURE EXTRACTION ---
def _extract_channel_features(sig):
    """sig: (N,) -> (2, N) = [sig_norm, local_rms]"""
    sig_norm  = (sig - np.mean(sig)) / (np.std(sig) + 1e-8)
    local_rms = np.sqrt(np.convolve(sig_norm ** 2, np.ones(50) / 50, mode='same'))
    return np.stack([sig_norm, local_rms], axis=0).astype(np.float32)


def extract_features_signal(sig):
    """sig: (6, N) -> (12, N), 2 features per channel."""
    return np.concatenate([_extract_channel_features(sig[ch]) for ch in range(sig.shape[0])], axis=0)
...
class ResidualBlock(nn.Module):
    def __init__(self, in_c, out_c):
        ...
        self.conv = nn.Sequential(
            ...
            nn.GroupNorm(4, out_c), nn.ReLU(), nn.Dropout(0.1),
...
class SignalLevelLoader:
    ...
    20-minute recording, sharing one movement mask. Unlike model 21/22, we do
    NOT split into 6 separate channel-signals; each recording is kept as a
    single (6, N) sample, fed to the network as 6 input channels.
    """
    def __init__(self, sig_paths, mask_paths):
        ...
        for sp, mp in zip(sig_paths, mask_paths):
            ...
            self.feats_list.append(extract_features_signal(sig))   # (6, N)
            self.masks_list.append(mask)

        print(f"  {len(self.feats_list)} recordings loaded.")
...
def main():
    ...
    DATA_ROOT = "/home/20251020/ECG/Npy_DB_2"
    ...
    optimizer = optim.Adam(model.parameters(), lr=LEARNING_RATE, betas=(0.95, 0.999), weight_decay=1e-4)
    ...
    print(f"Model 23 | 6 raw channels (sig_norm only, no engineered features) | self-attention")
    ...
    for epoch in range(EPOCHS):
        ...
        if f1 > best_f1:
            best_f1 = f1
            torch.save(model.state_dict(), "23_AttUNet_6ch_raw_best.pth")
            print(f"--> New best (F1={f1:.3f})  saved: 23_AttUNet_6ch_raw_best.pth")
