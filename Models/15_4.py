...
# Model 15_4: same architecture/loss as 15_2/15_3 (model 17's no-self-attention
# AttentionUNet, whole-signal normalization), but `curvature` is dropped and
# replaced with two features that analyze_classes_15_3.py found to be the
# best (still weak, but best) C1 (linear) vs C2 (spline) separators:
#   env_range -- per-window max-min of the smoothed QRS-peak envelope
#   env_slope -- per-window mean slope (1st derivative) of that envelope

NUM_CLASSES  = 4
NUM_FEATURES = 5   # residual, local_rms, qrs_rate, env_range, env_slope
SMOOTH_WINDOW = 50
...
def extract_features_signal(sig, qrs_locs):
    ...
    if len(qrs_locs) < 2:
        ...
        return feats, np.zeros(n), np.zeros(n)
    ...
    # smooth_env and its first derivative are used by the loader to compute
    # per-window env_range (max-min of smooth_env) and env_slope (mean slope)
    smooth_env = np.convolve(linear_env, np.ones(SMOOTH_WINDOW) / SMOOTH_WINDOW, mode='same')

    env_slope_point = np.gradient(smooth_env)
    env_slope_point[:SMOOTH_WINDOW]  = env_slope_point[SMOOTH_WINDOW]
    env_slope_point[-SMOOTH_WINDOW:] = env_slope_point[-SMOOTH_WINDOW - 1]

    # channels 3 (env_range) and 4 (env_slope) are placeholders here; the
    # loader fills them with per-window constants from smooth_env / env_slope_point
    feats = np.stack([
        residual, local_rms, qrs_rate,
        np.zeros(n), np.zeros(n)
    ], axis=0)

    return feats, smooth_env, env_slope_point
...
class SignalLevelLoader:
    ...
    def get_batches(self, window_size, stride, batch_size):
        ...
        for idx in indices:
            ...
            feats, smooth_env, env_slope_point = extract_features_signal(sig, qrs_locs)
            ...
            for start in range(0, sig.shape[0] - window_size, stride):
                ...
                env_win   = smooth_env[start:end]
                slope_win = env_slope_point[start:end]
                window_feats[ENV_RANGE_CHANNEL, :] = env_win.max() - env_win.min()
                window_feats[ENV_SLOPE_CHANNEL, :] = slope_win.mean()
...
def main():
    ...
    CHECKPOINT  = "15_4_AttUNet_multiclass_best.pth"
    ...
    print(f"Model 15_4 | Multiclass (4) | residual+rms+qrs_rate+env_range+env_slope | signal-level norm | LR: {LEARNING_RATE}")
